...
async function testArticleRuntimeCutover() {
  console.log('========================================================================');
  console.log('📰 DB-9: ARTICLE RUNTIME CUTOVER VERIFICATION & REGRESSION AUDIT');
  console.log('========================================================================\n');
  ...
  if (
    ...
    console.error('❌ Provider configuration failed! Doctor, Package, Category & Article must be Postgres, all others Static.');
    process.exit(1);
  }

  // 2. Test Article Live PostgreSQL Queries
  console.log('\n--- 2. ARTICLE REPOSITORY LIVE POSTGRESQL QUERIES ---');
  const allArticles = await articleRepository.getAll();
  console.log(`Fetched ${allArticles.length} articles from PostgreSQL.`);

  if (allArticles.length !== 108) {
    console.error(`❌ Expected 108 articles, got ${allArticles.length}`);
    process.exit(1);
  }
  ...
  for (const slug of sampleSlugs) {
    const art = await articleRepository.getBySlug(slug);
    if (!art) {
      console.error(`❌ Failed to retrieve article by slug "${slug}" from PostgreSQL!`);
      process.exit(1);
    }
    console.log(`  ✓ [ID ${art.id}] ${art.title.slice(0, 45)}... | Categories: [${art.categories.join(', ')}] | TOC: ${art.tableOfContents?.length || 0} items`);
  }

  // Test getById
  const sampleById = await articleRepository.getById(5637);
  if (!sampleById || sampleById.slug !== 'tu-kiem-dinh-quoc-te-den-thuc-hanh-hang-ngay-trien-khai-aaci-trong-mo-hinh-phong-kham-tai-doctor-check') {
    console.error('❌ getById(5637) failed!');
    process.exit(1);
  }
  console.log(`  ✓ getById(5637) returned: "${sampleById.title.slice(0, 40)}..."`);

  // Test getByCategory
  const articlesInCat49 = await articleRepository.getByCategory(49);
  console.log(`  ✓ getByCategory(49) returned ${articlesInCat49.length} articles.`);

  // Test getRecent
  const recentArticles = await articleRepository.getRecent(4);
  console.log(`  ✓ getRecent(4) returned ${recentArticles.length} articles.`);

  // Test Non-Existent Article Query
  const nonExistent = await articleRepository.getBySlug('non-existent-article-slug-xyz');
  if (nonExistent !== null) {
    console.error('❌ Non-existent article query did not return null');
    process.exit(1);
  }
  console.log('  ✓ getBySlug("non-existent-article-slug-xyz") returned null correctly.');

  // 3. PostgreSQL Regression Audit (Doctors, Packages, Categories)
  console.log('\n--- 3. POSTGRESQL REGRESSION AUDIT (DOCTORS, PACKAGES, CATEGORIES) ---');
  ...
  console.log('\n========================================================================');
  console.log('🎉 DB-9 ARTICLE RUNTIME CUTOVER VERIFICATION COMPLETED SUCCESSFULLY!');
  console.log('========================================================================\n');
  process.exit(0);
}

testArticleRuntimeCutover().catch((err) => {
  console.error('❌ Error during article runtime cutover testing:', err);
  process.exit(1);
});
