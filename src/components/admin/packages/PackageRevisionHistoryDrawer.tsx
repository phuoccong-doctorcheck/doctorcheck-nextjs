...
import { getPackageRevisionsAction, restorePackageRevisionAction } from '@/actions/package.actions';

interface PackageRevisionHistoryDrawerProps {
  entityId: string;
  isOpen: boolean;
  onClose: () => void;
...
export function PackageRevisionHistoryDrawer({
  entityId,
  isOpen,
  onClose,
  onRestored,
}: PackageRevisionHistoryDrawerProps) {
  ...
  useEffect(() => {
    ...
    async function loadRevisions() {
      ...
      setError(null);
      try {
        const res = await getPackageRevisionsAction(entityId);
        if (mounted) {
  ...
  const handleRestore = async (revisionId: string) => {
    ...
    setError(null);
    try {
      const res = await restorePackageRevisionAction(revisionId);
      if (res.success && res.data) {
          ...
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
                Lịch Sử Phiên Bản Gói Khám & Bảng Giá
              </h2>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {loading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-800 p-8 text-center text-slate-500">
                Chưa có bản ghi lịch sử phiên bản nào cho gói khám này.
              </div>
            ) : (
                    ...
                    >
                      ...
                      {isSelected && payload && (
                        <div className="mt-4 rounded-lg bg-slate-50 dark:bg-slate-800/60 p-3 text-xs space-y-1.5 font-mono text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                          <div>
                            <span className="text-slate-400">Gói khám: </span>
                          ...
                          <div>
                            ...
                            <span className="font-sans font-bold text-emerald-600 dark:text-emerald-400">
                              {String(payload.priceFormatted || `${Number(payload.priceVnd).toLocaleString('vi-VN')}đ`)}
                            </span>
                          </div>
                          <div>
                            <span className="text-slate-400">Đối tượng: </span>
                            <span>{String(payload.gender || 'both')}</span>
                          </div>
                          <div>
                            <span className="text-slate-400">Thời lượng: </span>
                            <span>{String(payload.duration || '120 - 180 phút')}</span>
                          </div>
                          <div>
                            <span className="text-slate-400">Số hạng mục / Features: </span>
                            <span>{Array.isArray(payload.features) ? payload.features.length : 0} hạng mục</span>
                          </div>
                        </div>
                      )}
                    </div>
