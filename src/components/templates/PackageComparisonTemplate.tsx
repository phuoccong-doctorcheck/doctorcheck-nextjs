...
export function PackageComparisonTemplate({ page, pageContent }: PackageComparisonTemplateProps) {
  ...
  const hasEmbeddedBooking =
    contentHtml.includes('wpcf7') ||
    contentHtml.includes('form-v3') ||
    contentHtml.includes('dc_form');
  ...
  return (
    <div className="package-family-page min-h-screen flex flex-col bg-[#FFFFFF] selection:bg-[#FFB500] selection:text-[#00475B]">
      ...
      <main id="main" className="flex-1 w-full">
        ...
        {/* 4. Clinical Assurance & Trust Banner (only when not in contentHtml) */}
        {!hasEmbeddedBooking && (
        ...
        {/* 5. Consultation & Booking Form (only when not in contentHtml) */}
        {!hasEmbeddedBooking && (
