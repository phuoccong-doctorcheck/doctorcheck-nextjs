...
interface ClinicalGuideTemplateProps {
  page: PageRouteMetadata;
  pageContent?: PageContent;
}

export function ClinicalGuideTemplate({ page, pageContent }: ClinicalGuideTemplateProps) {
  const isNestedSubpath = page.subpath && page.subpath.trim().length > 0;
  const contentHtml = pageContent?.contentHtml || '';

  return (
    <div className="clinical-guide-page min-h-screen flex flex-col bg-[#FFFFFF] selection:bg-[#FFB500] selection:text-[#00475B]">
      {/* 1. Global Header */}
      <Header />
