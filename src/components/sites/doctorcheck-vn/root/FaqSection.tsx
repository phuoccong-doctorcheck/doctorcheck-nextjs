...
const defaultFaqs: FaqDisplayItem[] = [
  {
    id: 'faq-1',
...
interface FaqSectionProps {
  faqsList?: FaqDisplayItem[];
  header?: SectionsMetaConfig['faqHeader'];
}

export function FaqSection({ faqsList, header }: FaqSectionProps = {}) {
  const [openId, setOpenId] = useState<string | null>(null);
  const faqs = faqsList || defaultFaqs;
  const title = header?.title || 'Câu hỏi thường gặp';
